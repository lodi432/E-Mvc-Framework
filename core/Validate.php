...
class Validate {
  ...
  public function displayErrors() {
    $html = '<div class="[success alert secondary] [round radius] label">';
    foreach ($this->_errors as $error){
      if(is_array($error)){
        $html .= '<li class="text">'.$error[0].'</li>';
        $html .= '<script>jQuery("document").ready(function(){jQuery("#'.$error[1].'").parent().closest("div").addClass("form-error.is-visible");});</script>';

      }else {
        $html.= '<li class="text-center">'.$error.'</li>';
      }
